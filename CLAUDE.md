...
5. **인증 금지선: 홈페이지 비밀번호를 Ondal이 직접 받는 구조 금지**
   - 연동: 홈페이지 Keycloak 로그인 - OIDC 인가 코드 + PKCE, BE 가 코드 교환, ID 토큰 클레임(username)으로 User 매칭 (docs 결정 7)
   - 모드 `ondal.auth.mode`: prod = `oidc`(`auth/oidc/`), local·test = `stub`(loginId 만으로 로그인, prod 에서는 기동 거부)
   - 세션·인터셉터·`@LoginUser` 는 모드와 무관하게 동일 - 인증 방식이 바뀌어도 도메인 코드는 손대지 않음
